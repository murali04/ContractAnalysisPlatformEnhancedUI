                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedObligation.cot_steps?.map((step: any, i: number) => (
                      <div key={i} className={`p-2.5 rounded-lg border text-xs flex items-center gap-3 ${step.status === 'FAIL' ? 'bg-rose-50/50 border-rose-100 text-rose-800' : 'bg-emerald-50/50 border-emerald-100 text-emerald-800'}`}>
                        <div className="mt-0.5">
                          {step.status === 'FAIL' ? <XCircle size={14} /> : <CheckCircle size={14} />}
                        </div>
                        <div className="flex-1 font-semibold flex justify-between items-center">
