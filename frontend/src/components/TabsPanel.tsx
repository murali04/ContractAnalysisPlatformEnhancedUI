...
export function TabsPanel({
  ...
  const stepDescriptions: Record<string, string> = {
    "Identify Obligation Purpose":
      "Determines the core intent (e.g., payment, indemnification).",
    "Analyze Clause Effect": "Evaluates the clause’s impact on the obligation.",
    "Match Analysis":
      "Checks whether the clause explicitly covers the obligation.",
    "Material Conflicts Check":
      "Ensures there are no contradictions in related terms.",
    "Termination Check":
      "Validates if contract termination affects this obligation.",
    "Discretion Check":
      "Looks for 'sole discretion' or similar language that weakens obligation.",
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === "details" && (
              <div className="space-y-6 fade-in">
                <div className="p-4 bg-linear-to-br ffrom-backdrop-linear-primary to-backdrop-linear-secondarrounded-xl border border-purple-200 shadow-sm">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900 mb-3 flex items-center gap-2">
                    <ListChecks size={14} className="text-tabs-title-primary" />{" "}
                    Validation Steps
                  </h4>
                  <div className="space-y-2">
                    {selectedObligation.cot_steps?.map(
                      (step: any, i: number) => {
